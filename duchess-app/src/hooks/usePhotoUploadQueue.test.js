...
describe('usePhotoUploadQueue P11D observation', () => {
  beforeEach(() => {
    global.IS_REACT_ACT_ENVIRONMENT = true
  })
  ...
  test('queueRecords filters actor, source surface, and entity', async () => {
    ...
    recordsById.set('q-wrong-entity', createRecord({ queueId: 'q-wrong-entity', status: PHOTO_UPLOAD_STATUSES.QUEUED, entityId: 'job-99' }))
    ...
    const createStore = () => ({ start: jest.fn(), stop: jest.fn(async () => {}) })
    const apiRef = { current: null }
    function Probe() {
      apiRef.current = usePhotoUploadQueue({
        jobId: JOB_ID,
  ...
  test('queueRecords DTO excludes blob, URLs, metadata, and raw error', async () => {
    ...
    expect(dto.source_surface).toBe(OFFICE_EVIDENCE_SOURCE_SURFACE)
    expect(dto.entity_type).toBe(OFFICE_EVIDENCE_ENTITY_TYPE)
